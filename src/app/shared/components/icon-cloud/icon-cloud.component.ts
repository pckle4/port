...
@Component({
  ...
  template: `
    <div #cloudContainer class="cloud-container w-full h-full min-h-[300px] flex items-center justify-center cursor-grab active:cursor-grabbing"></div>
  `,
  styles: [`
    :host {
      display: block;
...
export class IconCloudComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private initThreeJS() {
    this.ngZone.runOutsideAngular(() => {
      ...
      const processQueue = () => {
        ...
        while (inFlightRequests.size < 6 && !loadQueue.isEmpty()) {
      ...
      const lineMaterial = new THREE.LineBasicMaterial({ 
        color: 0x4a5568, // fallback line color
        transparent: true, 
        opacity: 0.1 
      });
  ...
  private async loadIconTexture(slug: string): Promise<THREE.Texture | null> {
    const isDark = true; // Use default dark styles for icons for now
  ...
  private animate() {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      if (this.isDragging) {
        ...
        if (avgMomentum > 0.5) {
          this.currentSpeed += (avgMomentum * 0.05); 
          this.momentumHistory.add(0); 
        }
