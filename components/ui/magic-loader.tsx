...
const MagicLoader: React.FC<MagicLoaderProps> = ({
  ...
  const drawParticle = useCallback((ctx: CanvasRenderingContext2D, particle: Particle, index: number, tick: number, currentLightness: number) => {

    const hueSpan = hueRange[1] - hueRange[0];
    const hue = hueSpan === 0 ? hueRange[0] : hueRange[0] + ((tick + (particle.life * 120)) % hueSpan);
    ctx.fillStyle = ctx.strokeStyle = `hsla(${hue}, ${saturation}%, ${currentLightness}%, ${particle.life})`;
  ...
  const animate = useCallback(() => {
    ...
    const isDark = hasClassDark || storedTheme === "dark" || (storedTheme === "system" && prefersDark) || (!storedTheme && prefersDark);
    const currentLightness = isDark ? 100 : 0;


    particlesRef.current.forEach((particle, index) => {
      drawParticle(ctx, particle, index, tickRef.current, currentLightness);
    });
