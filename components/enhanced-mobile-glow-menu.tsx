...
export function EnhancedMobileGlowMenu() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 md:hidden bg-black/70 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
      )}

      <div
        className={`fixed top-0 right-0 h-screen w-64 sm:w-72 z-50 md:hidden transform transition-transform duration-300 ease-in-out ${
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 rounded-lg transition-all duration-300 ${
                      isActive
                        ? "bg-gradient-to-r from-purple-600 via-blue-600 to-cyan-600 text-white"
