...
export function OptimizedHeader() {
  ...
  useEffect(() => {
    // Dynamic underline animation that repeats
    if (underlineRef.current) {
      const animateUnderline = () => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            if (!underlineRef.current) return
            underlineRef.current.style.transform = "scaleX(1) scaleY(1.2)"
            setTimeout(() => {
              if (!underlineRef.current) return
              underlineRef.current.style.transform = "scaleX(1) scaleY(1)"
            }, 200)
          }, 800)
        }, 100)
      }
      ...
      // Initial animation
      setTimeout(animateUnderline, 1000)

      // Repeat animation every 8 seconds
      const interval = setInterval(animateUnderline, 8000)
        ...
        <div className="flex justify-between items-center py-4">
          <div className="flex items-center">
            <Link
              ref={logoRef}
              href="/"
              className="flex items-center space-x-3 text-xl sm:text-2xl font-bold hover:scale-105 transition-transform group z-10"
            >
              ...
              <div className="relative">
                <span className="font-extrabold tracking-tight text-foreground drop-shadow-sm">Ansh Shah</span>
                ...
                <div
                  ref={underlineRef}
                  className="absolute -bottom-1 left-0 w-full h-1 bg-gradient-to-r from-purple-600 via-blue-600 to-purple-600 rounded-full opacity-0 transition-all duration-1000 ease-in-out"
                  style={{
          ...
          <nav className="hidden lg:flex space-x-8">
            {navigation.map((item) => (
              <Link
                ...
                className="text-foreground/80 hover:text-foreground transition-colors duration-200 relative group flex items-center space-x-1"
              >
